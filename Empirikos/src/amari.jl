...
#=
function modulus_cholesky_factor(convexclass, plugin_G, discr, eb_samples)
    K = nparams(convexclass)
    M = zeros(K, K)
    
    for _interval in discr
        for (z, pr) in zip(eb_samples.vec, eb_samples.probs)
            # Check 1: Probability validation
            if pr <= 0 || !isfinite(pr)
                @warn "Invalid probability" pr
                continue
            end
            
            z = set_response(z, _interval)
            
            # Check 2: Plugin density
            plugin_dens = pdf(plugin_G, z)
            if plugin_dens <= 0 || !isfinite(plugin_dens)
                @warn "Invalid plugin density" z=z dens=plugin_dens
                continue
            end
            
            log_plugin = log(plugin_dens)
            
            # Check 3: Component densities
            comp_densities = pdf.(components(convexclass), z)
            if any(d <= 0 for d in comp_densities) || any(!isfinite, comp_densities)
                @warn "Invalid component densities" z=z dens=comp_densities
                continue
            end
            
            log_vals = log.(comp_densities)
            log_val = log_vals .- log_plugin / 2
            
            # Check 4: Log values
            if any(!isfinite, log_val)
                @warn "Invalid log values" log_val
                continue
            end
            
            exp_vals = exp.(log_val)
            
            # Check 5: Exponentiated values
            if any(!isfinite, exp_vals)
                @warn "Invalid exp values" exp_vals
                continue
            end
            
            cache_vec = sqrt(pr) .* exp_vals
            
            # Check 6: Cache vector
            if any(!isfinite, cache_vec)
                @warn "Invalid cache vector" cache_vec
                continue
            end
            
            M .+= cache_vec * cache_vec'
        end
    end
    
    # Final check: Matrix validation
    if any(!isfinite, M)
        @error "Matrix contains NaN/Inf" M
    end
    
    cholesky(M + 1e-8 * I)  # Regularize
end
=#
