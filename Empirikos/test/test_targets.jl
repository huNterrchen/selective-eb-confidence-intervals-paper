...
computed = Empirikos.compute_target(Empirikos.Conjugate(), Empirikos.SymmetricPosteriorMean(Z), Z, prior)
...
function coverage_closed_form(z, μ, τ)
    σ = sqrt(1 + τ^2)
    spost = τ / σ
    mplus  = (τ^2*z + μ)/(1 + τ^2)
    ...
    pos = Φ(( z + 1.96 - mplus )/spost) - Φ(( z - 1.96 - mplus )/spost)
    neg = Φ((-z + 1.96 - mminus)/spost) - Φ((-z - 1.96 - mminus)/spost)

    (fz*pos + fneg*neg) / (fz + fneg)
end

@testset "CoverageProbability: Normal prior" begin
    #test for numerator functionality
    t   = CoverageProbability(FoldedNormalSample(4))
    t_num = numerator(t)
    p = Normal(1,2)
    positive_set = Interval(4 - 1.96, 4 + 1.96)
    negative_set = Interval(-4 - 1.96, -4 + 1.96)
    @test t_num(p) == numerator(Empirikos.PosteriorProbability(StandardNormalSample(4), positive_set))(p) +
                     numerator(Empirikos.PosteriorProbability(StandardNormalSample(-4), negative_set))(p)
    for (μ, τ) in ((0.0, 0.5), (0.0, 1.0), (0.4, 0.7), (-0.8, 1.2), (1.0, 2.0))
        prior = Normal(μ, τ)
        for z in (0.0, 0.25, 0.5, 1.0, 2.0, 3.0)
            targ = CoverageProbability(FoldedNormalSample(abs(z)))
            num = numerator(targ)(prior)
            den = denominator(targ)(prior)
            val_impl  = num / den
            val  = targ(prior)
            @test isapprox(val, val_impl; rtol=1e-11, atol=1e-12)
            val_exact = coverage_closed_form(abs(z), μ, τ)
            @test isapprox(val_impl, val_exact; rtol=1e-11, atol=1e-12)
        end
    end
    #test for uniform prior
    ...
    t   = CoverageProbability(FoldedNormalSample(4))
    z_val = 4
    num_pos, _ = quadgk(μ -> (μ > z_val - 1.96) * (μ < z_val + 1.96) * pdf(Normal(μ, 1), z_val) * 1/20, -10, 10)

    num_neg, _ = quadgk(μ -> (μ > -z_val - 1.96) * (μ < -z_val + 1.96) * pdf(Normal(μ, 1), -z_val) * 1/20, -10, 10)
    
    @test isapprox(numerator(t)(prior), num_pos + num_neg, atol=1e-6)

end

function condrepl_closed_form(z, μ, τ)
    σ = sqrt(1 + (τ / sqrt(1 + τ^2))^2)         
    mplus  = (τ^2 * z + μ) / (1 + τ^2)
    mminus = (-τ^2 * z + μ) / (1 + τ^2)

    fz   = pdf(Normal(μ, sqrt(1 + τ^2)),  z)
    fneg = pdf(Normal(μ, sqrt(1 + τ^2)), -z)
...
@testset "ConditionalReplicationProbability: Normal prior" begin
    for (μ, τ) in ((0.0, 0.5), (0.0, 1.0), (0.4, 0.7), (-0.8, 1.2), (1.0, 2.0))
        prior = Normal(μ, τ)
        for z in (0.0, 0.25, 0.5, 1.0, 2.0, 3.0)
            targ = ConditionalReplicationProbability(FoldedNormalSample(abs(z)))
            num  = numerator(targ)(prior)
            den  =  denominator(targ)(prior)
            val  = targ(prior)
            val_impl  = num / den
            @test isapprox(val, val_impl; rtol=1e-11, atol=1e-12)
            val_exact = condrepl_closed_form(abs(z), μ, τ)
            @test isapprox(val_impl, val_exact; rtol=1e-11, atol=1e-12)
        end
    end
end